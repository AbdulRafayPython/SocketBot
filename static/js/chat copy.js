document.addEventListener('DOMContentLoaded', () => {
    ...
    emojiPicker.style.zIndex = '1000'; // Ensure it appears above other elements
    emojiPicker.style.background = '#2c2f33'; // Match dark theme
    emojiPicker.style.borderRadius = '8px';
    emojiPicker.style.boxShadow = '0 4px 8px rgba(0, 0, 0, 0.3)';
    ...
    function positionEmojiPicker() {
        ...
        if (viewportWidth <= 768) {
            ...
            // Desktop: Position above emoji button
            emojiPicker.style.position = 'absolute';
            // Place above button, but ensure it doesn’t go off-screen
            const top = rect.top - pickerHeight < 0 ? rect.bottom + 5 : rect.top - pickerHeight;
            emojiPicker.style.top = `${top}px`;
            // Align left with button, but keep within viewport
            const left = Math.min(rect.left, viewportWidth - 320); // 320px is approximate picker width
